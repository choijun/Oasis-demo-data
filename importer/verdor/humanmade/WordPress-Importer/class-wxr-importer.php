...
class WXR_Importer extends WP_Importer {
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		foreach ( $meta as $meta_item ) {
			...
			$_wp_attached_file = $meta_item['value'];
			break;
		}
		...
		if(!empty($_wp_attached_file) && file_exists( $upload_dir['basedir'] . '/' . $_wp_attached_file)){
			$upload = array(
				'file' => $upload_dir['basedir'] . '/' . $_wp_attached_file,
				'url'  => $upload_dir['baseurl'] . '/' . $_wp_attached_file
			);
		}else{
