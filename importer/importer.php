...
if(!class_exists('WXR_Importer')){
	require dirname( __FILE__ ) . '/verdor/humanmade/WordPress-Importer/plugin.php';
}
...
class LaStudio_Importer {
	...
	protected function __construct() {
		...
		add_action( 'init', array( $this, 'setup_plugin_data' ) );
	...
	public static function admin_enqueue_scripts(){
		...
		wp_localize_script( 'lastudio-importer-js', 'lastudio_importer',
			array(
				...
				'loader_text' => esc_html__( 'Importing now, please wait!', LA_TEXTDOMAIN ),
			)
		);
	...
	public function action_process_importer(){
		...
		if( false === check_ajax_referer( 'lastudio-importer-security', 'security', false ) ) {
			wp_send_json( __( 'Access define!', LA_TEXTDOMAIN ) );
		}
	...
	protected function _start_ajax_importer_handling(){
		...
		wp_send_json(array(
			...
			'data'  => array(
				...
				'url' => $ajax_import_url,
				'strings' => array(
					'complete' => __( 'Import complete!', LA_TEXTDOMAIN ),
				)
			)
		));
	...
	public static function data_from_file( $file_path ) {
		...
		if ( ! $data ) {
			return new WP_Error(
				...
				sprintf(
					__( 'An error occurred while reading a file from your server! Tried reading file from path: %s%s.', LA_TEXTDOMAIN ),
					'<br>',
					$file_path
				)
			);
	...
	private static function return_direct_filesystem_error() {
		return new WP_Error(
			...
			sprintf(
				__( 'This WordPress page does not have %sdirect%s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %s.', LA_TEXTDOMAIN ),
				'<strong>',
				'</strong>',
	...
	private static function check_wp_filesystem_credentials() {
		...
		if ( false === ( $creds = request_filesystem_credentials( $demo_import_page_url, '', false, false, null ) ) ) {
			return new WP_error(
				...
				__( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', LA_TEXTDOMAIN )
			);
		}
		...
		if ( ! WP_Filesystem( $creds ) ) {
			return new WP_Error(
				...
				__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', LA_TEXTDOMAIN )
			);
		}
	...
	private function handling_importer_widgets( $file ) {
		...
		$response = array(
			...
			'error'  => __( 'Widget import file could not be found.', LA_TEXTDOMAIN )
		);
		...
		if ( is_wp_error( $result ) ) {
			...
			$this->logger->info(__('Widget has been importer !', LA_TEXTDOMAIN));
			$response['error'] = __( 'Widget has been importer !', LA_TEXTDOMAIN);
		}
		$this->emit_sse_message( $response );
		return;
	...
	private function import_widget_data( $data ) {
		...
		if ( empty( $data ) || ! is_object( $data ) ) {
			return new WP_Error(
				...
				__( 'Widget import data could not be read. Please try a different file.', LA_TEXTDOMAIN )
			);
		}
		// Hook before import.
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			if ( isset( $wp_registered_sidebars[ $sidebar_id ] ) ) {
				...
				$sidebar_message      = __( 'Sidebar does not exist in theme (moving widget to Inactive)', LA_TEXTDOMAIN );
			}
			// Result for sidebar.
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				if ( ! $fail && ! isset( $available_widgets[ $id_base ] ) ) {
					...
					$widget_message      = __( 'Site does not support widget', LA_TEXTDOMAIN ); // Explain why widget not imported.
				}
				...
				if ( ! $fail && isset( $widget_instances[ $id_base ] ) ) {
					...
					foreach ( $single_widget_instances as $check_id => $check_widget ) {
						...
						if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
							...
							$widget_message      = __( 'Widget already exists', LA_TEXTDOMAIN ); // Explain why widget not imported.
				...
				if ( ! $fail ) {
					...
					if ( $sidebar_available ) {
						...
						$widget_message      = __( 'Imported', LA_TEXTDOMAIN );
					}
					else {
						$widget_message_type = 'warning';
						$widget_message      = __( 'Imported to Inactive', LA_TEXTDOMAIN );
					}
				}

				// Result for widget instance.
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['name']         = isset( $available_widgets[ $id_base ]['name'] ) ? $available_widgets[ $id_base ]['name'] : $id_base; // Widget name or ID if name not available (not supported by site).
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['title']        = ! empty( $widget['title'] ) ? $widget['title'] : __( '', LA_TEXTDOMAIN ); // Show "No Title" if widget instance is untitled.
		...
		if ( !empty( $file ) && file_exists( $file ) ) {
			if( class_exists('RevSlider') ) {
				...
				if( is_wp_error( $result ) ) {
					$response['error'] = $result->get_error_message();
					$this->logger->error(
						sprintf(__('ImportingSlider %s', LA_TEXTDOMAIN), $result->get_error_message())
					);
				}else{
					$this->logger->info(
						__('Slider has been imported !', LA_TEXTDOMAIN)
					);
				}
			}
		}
	...
	private function handling_importer_option( $file ) {
		...
		if( is_wp_error( $data ) ) {
			$this->logger->error(
				__('Option config not found!', LA_TEXTDOMAIN)
			);
			return;
		}
		...
		if( empty( $data ) || !is_array( $data ) ) {
			$this->emit_sse_message( array(
				'action' => 'ImportingOption',
				'error'  => __( 'Options is null', LA_TEXTDOMAIN)
			) );
			return;
		}
	...
	private function handling_importer_theme_mode( $demo_id ) {
		...
		if(!empty($menu_locations)){
			...
			$this->logger->info(
				__('Menu Location has been set', LA_TEXTDOMAIN)
			);
		}
		$pages = array();
		...
		if(!empty($pages)){
			...
			$this->logger->info(
				__('Home Page and Blog Page has been set!', LA_TEXTDOMAIN)
			);
		}
		...
		$this->logger->info(
			__('Active demo success !', LA_TEXTDOMAIN)
		);
